...
/** Sync stage dot with the current `<select>` value. */
...
export function buildMapPinStageSelectHtml(slug: string, stage: LeadStage): string {
  ...
  return `<div class="map-pin-stage-row" style="margin-top:8px;">
    ...
    <div style="position:relative;">
      ...
      <select class="map-pin-stage-select" data-slug="${escapeHtml(slug)}" style="width:100%;font-size:12px;border:1px solid #cbd5e1;border-radius:6px;padding:4px 8px 4px 26px;background:#fff;color:#0f172a;">
        ${options}
      </select>
    </div>
  </div>`;
