					...
					{tasksOpen ? (
						<ul className="mt-2 space-y-2">
							{project.openTasks.map((task) => (
								<li
									key={task.id}
									className="client-portal-task rounded-lg bg-white/[0.03] px-3 py-2 text-xs"
								>
		...
		>
			...
			<div className="text-xs text-slate-400">
				...
				{project.handoverDate ? (
					<p className="text-emerald-400 client-portal-accent-emerald">
						Handover {formatShortDate(project.handoverDate)}
					</p>
				) : null}
			</div>
			{outcome ? (
...
export function ClientPortalClient({
	...
	const openTaskCount = data.activeProjects.reduce(
		(n, p) => n + p.openTasks.length,
		0,
	);
							...
							<section>
								...
								<div className="client-portal-card mt-3 rounded-xl border border-white/[0.08] bg-white/[0.03] p-4">
									...
									<div className="mt-4 flex flex-wrap gap-x-3 gap-y-1 text-[10px] text-slate-500">
										...
										<span className="inline-flex items-center gap-1">
											<span className="h-2 w-2 rounded-full bg-amber-500" /> 1–3
											days
										</span>
										<span className="inline-flex items-center gap-1">
										...
										<span className="inline-flex items-center gap-1">
											<span className="h-2 w-2 rounded-full bg-sky-500" /> 8–14
											days
										</span>
										<span className="inline-flex items-center gap-1">
								...
								{openTaskCount > 0 ? (
									<p className="mt-3 text-xs text-slate-500">
										{openTaskCount} open kanban task
										{openTaskCount === 1 ? "" : "s"} across active projects
									</p>
								) : null}
							</section>
